...
module.exports = async (event, context) => {
  ...
  if (deviceKey && deviceID) {
    const { rows } = await client.query(
      'SELECT device_id, device_key FROM device WHERE device_id = $1 and device_key = $2',
      [deviceID, deviceKey]
    );
    if (rows.length) {
      ...
      return context
        .status(401)
        .fail({ status: 'invalid authorization or device' });
    }
  }
...
async function insertStatus(deviceID, event, client) {
  ...
  try {
    let res = await client.query(
      'INSERT INTO device_status (device_id, uptime, temperature_c) values($1, $2, $3)',
